...
FULL_YOLO_BACKEND_PATH = "model/init/yolo.weights"   # should be hosted on a server
...
TINY_DARKNET_BACKEND_PATH = "detect_model/tiny.weights"
...
class FullYoloFeature(BaseFeatureExtractor):
    ...
    def __init__(self, input_size):
        ...
        self.load_weights(self.feature_extractor, FULL_YOLO_BACKEND_PATH)
...
class TinyDarknetFeature(BaseFeatureExtractor):
    """docstring for ClassName"""
    def __init__(self, input_size):
        input_image = Input(shape=(input_size, input_size, 3))
        ...
        self.feature_extractor = Model(input_image, x)
        self.load_weights(self.feature_extractor, TINY_DARKNET_BACKEND_PATH)
        for i in range(16, 19):
            layer = self.feature_extractor.get_layer('conv_{}'.format(i))
            weights = layer.get_weights()
            new_kernel = np.random.normal(loc=0.0, scale=0.05, size=weights[0].shape)
